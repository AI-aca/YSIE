...
🚨[제약 사항]
1. 불릿 포인트(-)를 사용하는 항목(강점, 단점, 수정제안)의 개수는 문항의 형태와 무관하게 **반드시 항목당 4~5개의 문장(불릿)**을 꽉 채워 아주 상세하게 출력하십시오. (LLM 자체 판단으로 분량을 줄이는 행위를 절대 금지합니다.)
2. 4가지 항목(강점, 단점, 수정제안, 총평) 전체의 모든 문장은 반드시 **정중한 경어체(~습니다, ~합니다, ~바랍니다 등)**를 사용하여 작성하세요. 불릿 포인트 내용 작성 시에도 명사형 종결(~음, ~함)이나 반말은 절대 사용하지 마세요.
